...
class EnhancedTrustWrapper:
    ...
    def __init__(
        ...
        self.zk_generator = (
            create_zk_proof_generator(zk_network) if enable_zk_proofs else None
        )
        ...
        print("✅ Enhanced TrustWrapper initialized:")
        print(
            f"   AI Services: {', '.join(self.hallucination_detector.available_services)}"
        )
        print(f"   ZK Proofs: {'Enabled' if enable_zk_proofs else 'Disabled'}")
        print(
            f"   Leo Available: {self.zk_generator.leo_available if self.zk_generator else False}"
        )
    ...
    async def verified_execute(self, *args, **kwargs) -> EnhancedVerifiedResult:
        ...
        hallucination_result = await self.hallucination_detector.detect_hallucinations(
            str(response), {"args": args, "kwargs": kwargs}
        )
        ...
        verification_method = (
            "consensus"
            if len(self.hallucination_detector.available_services) > 1
            else "ai"
        )
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        hallucination_rate = self.hallucination_detections / max(
            self.execution_count, 1
        )
        ...
        return {
            ...
            "ai_services_available": len(
                self.hallucination_detector.available_services
            ),
            "zk_proofs_enabled": self.enable_zk_proofs,
            "leo_available": (
                self.zk_generator.leo_available if self.zk_generator else False
            ),
        }
...
# Factory functions
def create_enhanced_trust_wrapper(
    model: Any,
    model_name: Optional[str] = None,
    enable_zk_proofs: bool = True,
    zk_network: str = "testnet",
) -> EnhancedTrustWrapper:
