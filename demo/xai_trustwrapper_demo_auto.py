...
Auto-running version (no user input required)
"""
import sys
import os
import time
...
def auto_wait(seconds=2):
    """Auto-wait instead of user input"""
    print(f"\n{Colors.DIM}[Continuing in {seconds} seconds...]{Colors.RESET}")
    time.sleep(seconds)
...
def main():
    """Run the enhanced demo automatically"""
    try:
        # Introduction
        show_intro()
        ...
        print(f"\n{Colors.DIM}Demo complete. Will restart in 10 seconds...{Colors.RESET}\n")
