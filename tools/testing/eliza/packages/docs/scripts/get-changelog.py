...
def process_content_block(content):
    ...
    content = content.replace(
        "- {{maxTweetLength}} doesn't work in tweet post template",
        "- maxTweetLength doesn't work in tweet post template",
    )
    ...
    section_headers = [
        ...
        r"^## (Major changes.*?)$",
        r"^## (New Features.*?)$",
        r"^## (New Fixes.*?)$" r"^## (New Contributors.*?)$",
    ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Append new releases to an existing changelog from GitHub"
    )
    parser.add_argument(
        "--repo", default="elizaOS/eliza", help="GitHub repository in format owner/repo"
    )
    parser.add_argument("--token", help="GitHub personal access token")
    parser.add_argument(
        "--output", default="docs/changelog.md", help="Output file path"
    )
    parser.add_argument(
        "--force-rebuild",
        action="store_true",
        help="Rebuild the entire changelog instead of just appending",
    )
    ...
    if args.force_rebuild or not existing_content:
        ...
        print(
            f"Changelog updated with {len(new_releases)} new releases and saved to {args.output}"
        )
