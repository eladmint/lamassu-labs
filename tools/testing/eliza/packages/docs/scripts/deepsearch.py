...
MODEL = (
    "perplexity/sonar-reasoning-pro:online"  # Using the online variant for web search
)
...
def extract_frontmatter(content):
    ...
    if content.startswith("---"):
        ...
        if len(parts) >= 3:
            raw_front_matter_block = (
                f"---{parts[1]}---"  # Keep the raw block including delimiters
            )
            try:
                ...
                if not isinstance(front_matter_dict, dict):
                    ...
                    print(
                        f"Warning: Frontmatter parsed, but is not a dictionary: {type(front_matter_dict)}"
                    )
...
def create_enhanced_markdown(partner_info, research_results):
    ...
    title_text = (
        title_match.group(1) if title_match else front_matter_dict.get("title", "")
    )
    title_heading = f"# {title_text}" if title_text else ""

    logo_match = re.search(
        r'<div className="partner-logo">.*?</div>', original_content, re.DOTALL
    )
...
def process_partners(root_dir):
    ...
    partner_folders = [
        d for d in root_path.iterdir() if d.is_dir() and (d / "index.md").exists()
    ]
        ...
        results.append(
            {
                ...
                "status": (
                    "Success" if len(enhanced_markdown) > 100 else "Possible Error"
                ),
            }
        )
