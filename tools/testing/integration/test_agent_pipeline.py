...
class TestAgentPipelineIntegration:
    ...
    async def test_three_stage_pipeline(
        self, link_finder_agent, data_extractor_agent, data_enricher_agent
    ):
    ...
    async def test_pipeline_performance_monitoring(self, mock_region_manager):
        ...
        assert pipeline_duration >= (
            extract_result.execution_time + enrich_result.execution_time
        )
    ...
    async def test_pipeline_anti_bot_coordination(self, link_finder_agent):
        ...
        original_create_session = (
            link_finder_agent.evasion_manager.create_evasive_session
        )
...
class TestAgentPipelineOrchestration:
    ...
    async def test_parallel_pipeline_execution(self, mock_region_manager):
        ...
        for i, result in enumerate(results):
            expected_batch = i // 3
            assert (
                result.data["event_details"]["title"]
                == f"Event from https://example.com/event-{i}"
            )
