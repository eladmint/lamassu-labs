...
<<<<<<< HEAD
    ...
    print(
        f"Response flagged as: {'⚠️ UNTRUSTED' if validated['hallucination_detection']['has_hallucination'] else '✓ TRUSTED'}"
    )
    print(f"Trust Score: {validated['final_trust_score']:.1%}")
    print(
        "Reason: Statistical hallucination detected - purple eyes don't occur naturally"
    )
    ...
    print("\n✅ With TrustWrapper Protection:")
    print(
        f"Alert: {'🚨 FABRICATION DETECTED' if validated['hallucination_detection']['has_hallucination'] else 'Verified'}"
    )
    print(f"Trust Score: {validated['final_trust_score']:.1%}")
    print("Detection: No such algorithm exists - likely scam")
    ...
    print("\n✅ With TrustWrapper Protection:")
    print(
        f"Warning: {'⚠️ NON-EXISTENT API' if validated['hallucination_detection']['has_hallucination'] else 'Valid API'}"
    )
    print(f"Trust Score: {validated['final_trust_score']:.1%}")
    print("Alert: torch.quantum module doesn't exist")
...
<<<<<<< HEAD
    ...
    print(
        f"📈 Improvement: {((caught_with - caught_without) / max(caught_without, 1) * 100):.0f}% better protection"
    )
