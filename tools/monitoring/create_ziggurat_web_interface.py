...
class ZigguratWebInterface:
        ...
        try:
            async with ICPClient(satellite_id=self.satellite_id) as client:
                ...
                if config_result["success"]:
                    ...
                    print(
                        f"❌ Failed to store web configuration: {config_result.get('error')}"
                    )
                    return False
                ...
                if dashboard_result["success"]:
                    print(
                        f"✅ Dashboard route stored: {dashboard_result['storage_id']}"
                    )
                else:
                    print(
                        f"❌ Failed to store dashboard route: {dashboard_result.get('error')}"
                    )
                    return False
                ...
                if routes_result["success"]:
                    ...
                    print(
                        f"❌ Failed to store routing table: {routes_result.get('error')}"
                    )
                    return False

                print("\n🎉 Custom web interface created successfully!")
                print("🌐 Access your monitoring dashboard at:")
    ...
    async def _create_web_configuration(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "routes": {
                "/": {"handler": "index_page", "method": "GET", "cache_ttl": 300},
                "/dashboard": {
    ...
    async def _create_dashboard_route(self) -> str:
        ...
        if dashboard_path.exists():
            ...
            modified_dashboard = (
                ...
                .replace(
                    "ICP-Powered Aleo Contract Monitor", "Lamassu Labs Contract Monitor"
                )
                .replace(
