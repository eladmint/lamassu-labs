...
from core.enhanced_trust_wrapper import (
    create_enhanced_trust_wrapper,
)
...
class TextValidationRequest(BaseModel):
    ...
    context: Optional[Dict[str, Any]] = Field(
        None, description="Optional context for validation"
    )
...
class BatchValidationRequest(BaseModel):
    texts: List[str] = Field(
        ..., max_items=10, description="Up to 10 texts to validate"
    )
    enable_zk_proofs: bool = Field(True, description="Enable ZK proof generation")
...
async def get_model(model_name: str = "default"):
    ...
    if model_name not in app.state.models:
        ...
        app.state.models[model_name] = create_enhanced_trust_wrapper(
            model=base_model, model_name=model_name, enable_zk_proofs=True
        )
...
if HAS_FASTAPI:
    ...
    async def startup_event():
        ...
        print(
            f"✅ API ready with services: {', '.join(app.state.detector.available_services)}"
        )
    ...
    async def health_check():
        ...
        return HealthResponse(
            ...
            leo_available=(
                default_model.zk_generator.leo_available
                if default_model.zk_generator
                else False
            ),
    ...
    async def validate_text(
        ...
        try:
            ...
            return ValidationResponse(
                text=request.text,
                has_hallucination=result["hallucination_detection"][
                    "has_hallucination"
                ],
                trust_score=result["trust_score"],
                hallucinations=result["hallucination_detection"]["hallucinations"],
                detection_time_ms=result["hallucination_detection"][
                    "detection_time_ms"
                ],
        ...
        try:
            ...
            batch_stats = {
                ...
                "hallucinations_detected": sum(
                    1 for r in results if r.has_hallucination
                ),
                "average_trust_score": sum(r.trust_score for r in results)
                / len(results),
                "average_detection_time_ms": sum(r.detection_time_ms for r in results)
                / len(results),
            }
    ...
    async def get_verification_stats(api_key: str = Depends(verify_api_key)):
        ...
        try:
            ...
            if model.zk_generator:
                total_verifications, total_hallucinations = (
                    await model.zk_generator.get_verification_stats()
                )
                return {
            ...
            raise HTTPException(
                status_code=500, detail=f"Verification stats failed: {str(e)}"
            )
...
async def log_validation(validation_type: str, text: str, result: Dict[str, Any]):
    ...
    print(
        f"[ANALYTICS] {validation_type} validation: {len(text)} chars, "
...
async def log_batch_validation(texts: List[str], results: List[ValidationResponse]):
    ...
    print(
        f"[ANALYTICS] Batch validation: {len(texts)} texts, "
...
async def log_model_query(query: str, result):
    ...
    print(
        f"[ANALYTICS] Model query: {len(query)} chars, "
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "trustwrapper_api:app", host="0.0.0.0", port=8000, reload=True, log_level="info"
    )
else:
