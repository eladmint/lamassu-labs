...
from .trust_wrapper_xai import ZKTrustWrapperXAI
...
class HallucinationDetectionResult:
    ...
    def add_hallucination(self, evidence: HallucinationEvidence):
        ...
        self.trust_score -= (
            severity_penalty.get(evidence.type, 0.3) * evidence.confidence
        )
...
class HallucinationDetector:
    ...
    def __init__(self):
        ...
        self.stat_pattern = re.compile(
            r"\b\d+(?:\.\d+)?%|\b\d+(?:,\d{3})*(?:\.\d+)?\s*(?:million|billion|thousand)"
        )
    ...
    async def _detect_factual_errors(
        self, text: str, context: Optional[Dict[str, Any]]
    ) -> List[HallucinationEvidence]:
        ...
        for country, capital in self.known_facts["capital"].items():
            ...
            for pattern in incorrect_patterns:
                if pattern in text_lower:
                    ...
                    claimed_capital = (
                        text_lower[start_idx : start_idx + 50]
                        .strip()
                        .split()[0]
                        .rstrip(".,;")
                    )
                    ...
                    if claimed_capital and claimed_capital != capital:
                        evidences.append(
                            HallucinationEvidence(
                                ...
                                source_text=text[
                                    max(0, start_idx - 50) : start_idx + 50
                                ],
                                evidence=[
                                    f"Known fact: {country.title()} -> {capital.title()}"
                                ],
                            )
                        )
    ...
    async def _detect_fabricated_citations(
        self, text: str
    ) -> List[HallucinationEvidence]:
                ...
                if any(keyword in title.lower() for keyword in suspicious_keywords):
                    evidences.append(
                        HallucinationEvidence(
                            ...
                            confidence=0.75,
                            description="Potentially fabricated citation with suspicious title",
                            source_text=(
                                f"{title} ({year})"
                                if title
                                else f"Citation from {year}"
                            ),
                            evidence=[
                                "Title contains hyperbolic language",
                                "Recent publication difficult to verify",
                            ],
                        )
                    )
    ...
    async def _detect_temporal_errors(
        self, text: str, context: Optional[Dict[str, Any]]
    ) -> List[HallucinationEvidence]:
        ...
        for year_str in year_mentions:
            ...
            if year > current_year:
                ...
                if any(
                    past_word in surrounding_text.lower()
                    for past_word in ["was", "happened", "occurred", "took place"]
                ):
                    evidences.append(
                        HallucinationEvidence(
                            ...
                            evidence=[
                                ...
                                "Future event described in past tense",
                            ],
                        )
                    )
            ...
            if "." in stat and len(stat.split(".")[1]) > 2:
                evidences.append(
                    HallucinationEvidence(
                        ...
                        evidence=[
                            "Overly precise decimal places",
                            "Lacks supporting source",
                        ],
                    )
                )
    ...
    async def _detect_confident_fabrications(
        self, text: str
    ) -> List[HallucinationEvidence]:
                ...
                if any(
                    ...
                    evidences.append(
                        HallucinationEvidence(
                            ...
                            evidence=[
                                "Contains absolute claim",
                                "High confidence indicator present",
                            ],
                        )
                    )
    ...
    def _get_surrounding_text(
        self, text: str, target: str, context_size: int = 50
    ) -> str:
...
class TrustWrapperValidator:
    ...
    def __init__(
        self, model: Any, enable_xai: bool = True, verification_level: str = "strict"
    ):
    ...
    async def validate_response(
        ...
        response_text = str(
            wrapped_result.data if wrapped_result.data else base_response
        )
        ...
        return {
            ...
            "xai_explanation": (
                wrapped_result.explanation.to_dict()
                if wrapped_result.explanation
                else None
            ),
