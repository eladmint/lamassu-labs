...
class TestCase:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "hallucination_type": (
                self.hallucination_type.value if self.hallucination_type else None
            ),
        }
...
class HallucinationTestSuite:
    ...
    def get_tests_by_type(
        self, hallucination_type: HallucinationType
    ) -> List[TestCase]:
        """Get test cases for specific hallucination type"""
        return [
            tc
            for tc in self.get_all_tests()
            if tc.hallucination_type == hallucination_type
        ]
    ...
    async def run_test_case(
        self, test_case: TestCase, model: Any, validator: TrustWrapperValidator
    ) -> Dict[str, Any]:
        ...
        result = await validator.validate_response(
            test_case.query, {"test_case": test_case.to_dict()}
        )
        ...
        triggers_found = any(
            trigger.lower() in response_text
            for trigger in test_case.known_hallucination_triggers
        )
    ...
    async def run_category(
        self, category_name: str, model: Any, validator: TrustWrapperValidator
    ) -> Dict[str, Any]:
            ...
            self.metrics[category_name].append(
                {
                    ...
                    "hallucinations_detected": result["hallucination_detection"][
                        "has_hallucination"
                    ],
                }
            )
    ...
    async def run_full_suite(
        self, model: Any, validator: TrustWrapperValidator
    ) -> Dict[str, Any]:
        ...
        for category_name in self.test_categories.keys():
            category_results[category_name] = await self.run_category(
                category_name, model, validator
            )
...
"""
        ...
        if results["detection_metrics"]["recall"] < 0.90:
            report += (
                "- Recall below target: Improve hallucination detection coverage\n"
            )
