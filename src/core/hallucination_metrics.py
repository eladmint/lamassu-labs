...
class HallucinationMetrics:
    ...
    # Performance metrics
    detection_latencies: List[float] = field(default_factory=list)  # Time to detect in ms
    trust_scores: List[float] = field(default_factory=list)         # Distribution of trust scores
    proof_generation_times: List[float] = field(default_factory=list) # ZK proof overhead in ms
    
    # Hallucination type breakdown
    type_counts: Dict[str, int] = field(default_factory=lambda: defaultdict(int))
    type_detection_rates: Dict[str, float] = field(default_factory=lambda: defaultdict(float))
    
    # Model-specific metrics
    model_metrics: Dict[str, Dict[str, Any]] = field(default_factory=lambda: defaultdict(dict))
    ...
    def avg_detection_latency(self) -> float:
        ...
        return statistics.mean(self.detection_latencies) if self.detection_latencies else 0.0
    ...
    def update_detection(self, predicted_hallucination: bool, actual_hallucination: bool, 
                        latency_ms: float, trust_score: float, proof_time_ms: float,
        ...
        if hallucination_type:
            ...
            if predicted_hallucination and actual_hallucination:
                ...
                self.type_detection_rates[hallucination_type] = (
                    (current_rate * (current_count - 1) + 1.0) / current_count
                )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'performance_metrics': {
                ...
                'avg_proof_generation_ms': statistics.mean(self.proof_generation_times) if self.proof_generation_times else 0,
            ...
            'total_evaluations': self.true_positives + self.false_positives + self.true_negatives + self.false_negatives
        }
    ...
    def meets_minimum_criteria(self) -> Tuple[bool, List[str]]:
        ...
        if self.avg_detection_latency > 200:
            failures.append(f"Avg latency {self.avg_detection_latency:.0f}ms > 200ms limit")
    ...
    def meets_target_criteria(self) -> Tuple[bool, List[str]]:
        ...
        if self.avg_detection_latency > 100:
            failures.append(f"Avg latency {self.avg_detection_latency:.0f}ms > 100ms target")
...
class PerformanceAnalyzer:
    ...
    async def measure_performance_impact(self, base_model: Any, wrapped_model: Any, 
                                       test_queries: List[str], runs_per_query: int = 10) -> Dict[str, Any]:
        ...
        for query in test_queries:
            ...
            results['per_query_analysis'].append({
                ...
                'std_dev_baseline': statistics.stdev(query_baselines) if len(query_baselines) > 1 else 0,
                'std_dev_wrapped': statistics.stdev(query_wrapped) if len(query_wrapped) > 1 else 0
            })
    ...
    async def test_scalability(self, wrapped_model: Any, test_queries: List[str], 
...
class A_B_TestFramework:
    ...
    def record_interaction(self, user_id: str, query: str, response: str, 
                         hallucination_detected: bool, user_satisfaction: Optional[float] = None):
        ...
        metrics = self.treatment_metrics if group == "treatment" else self.control_metrics
