...
class LeoProofGenerator:
    ...
    def __init__(self, program_id: str = 'trust_verifier.aleo', private_key: Optional[str] = None, view_key: Optional[str] = None):
        """
    ...
    def _find_contract_path(self) -> Optional[Path]:
        ...
        possible_paths = [
            Path(__file__).parent.parent / 'contracts' / f'{self.program_id.split(".")[0]}.leo',
            Path(__file__).parent.parent.parent / 'src' / 'contracts' / f'{self.program_id.split(".")[0]}.leo',
        ]
    ...
    async def generate_proof(self, 
                           ...
                           private_inputs: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            proof_data = await self._execute_leo_prove(function_name or 'verify_execution', leo_inputs)
    ...
    def _format_inputs_for_leo(self, inputs: Dict[str, Any]) -> str:
        ...
        for key, value in inputs.items():
            if isinstance(value, bool):
                ...
                if any(value.endswith(suffix) for suffix in ['field', 'u32', 'u64', 'i32', 'i64']):
    ...
    async def _execute_leo_prove(self, function_name: str, inputs: str) -> Dict[str, Any]:
        """
    ...
    async def verify_proof(self, proof_or_tx_id) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(proof_or_tx_id, 'proof_bytes'):
                valid = (
                    proof_or_tx_id.proof_bytes is not None and
                    len(proof_or_tx_id.proof_bytes) > 0 and
                    proof_or_tx_id.public_inputs is not None
                )
                return {
                    "valid": valid,
    ...
    async def generate_execution_proof(self,
                                     agent_hash: str,
                                     execution_time: int,
                                     success: bool,
                                     metrics_commitment: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    async def generate_batch_proof(self,
                                 agent_hash: str,
